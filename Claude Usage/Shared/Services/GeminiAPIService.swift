...
final class GeminiAPIService {
    ...
    func fetchUsage(credentials: GeminiCredentials) async throws -> GeminiUsage {
        ...
        // Step 2: If project ID provided, try Cloud Monitoring for usage metrics
        ...
        if let projectId = credentials.projectId {
            do {
                ...
                LoggingService.shared.log("GeminiAPIService: Cloud metrics fetched: \(requestCount ?? 0) requests, \(tokenCount ?? 0) tokens")
            } catch {
